...
<div class="container">
	<h1>Nuevo Alumno de Sociales</h1>
    <?php
    if(isset($_POST['enviar'])){  /* Verifica si el formulario fue enviado */
        ...
        $sql = "INSERT INTO sociales(nombre, app, apm, dir, num, col, telefono, mail, cuatrimestre)
                VALUES ('$nombre', '$app', '$apm', '$dir', '$num', '$col', '$telefono', '$mail', '$cuatrimestre')";
        
                // Verificar si el teléfono ya está registrado
        $sql_telefono = "SELECT * FROM sociales WHERE telefono = '$telefono'";
        ...
        if (mysqli_num_rows($resultado_telefono) > 0) {
            ...
            $sql_mail = "SELECT * FROM sociales WHERE mail = '$mail'";
    ...
    } else {  /* Si el formulario no ha sido enviado, muestra el formulario */
    ?>
        <form action="<?=$_SERVER['PHP_SELF']?>" method="post">  <!-- Inicia el formulario -->
            <table>  <!-- Tabla para organizar los campos -->
                <tr>
                ...
                <tr>
				   ...
				   <td> <input type="text" id="num" name="num" required  maxlength="3"/>
                <span id="mensaje-error-casa" style="color:red; display:none;">El número de casa debe ser un número o 'S/N'.</span></td>
                </tr>
                <tr>
                ...
                <tr>
                   ...
                   <span id="mensaje-error" style="color:red; display:none;">El teléfono debe tener 10 dígitos y no puede ser negativo.</span></td>
                </tr>
                <tr>
            ...
            <div class="form-buttons">
                ...
                <input type="button" value="Ver lista" onclick="window.location.href='sociales.php';" />
	...
	<script>
	// Validación para el número de casa
    document.getElementById('num').addEventListener('input', function(event) {
        var num = event.target.value;
        var mensajeErrorCasa = document.getElementById('mensaje-error-casa');

        // Validar que el número de casa sea un número o "S/N"
        if (num !== 'S/N' && !/^\d+$/.test(num)) {
            mensajeErrorCasa.style.display = 'block'; // Mostrar mensaje de error
        } else {
            mensajeErrorCasa.style.display = 'none'; // Ocultar mensaje de error
        }
    });
	</script>
