...
if (empty($cuatrimestre)) {
    ...
    $sql = "SELECT derecho.id, derecho.nombre AS derecho, derecho.app AS appe, derecho.apm AS apme 
            FROM derecho";
} else {
    // Si hay un cuatrimestre, se filtra por cuatrimestre
    $sql = "SELECT derecho.id, derecho.nombre AS derecho, derecho.app AS appe, derecho.apm AS apme 
            FROM derecho WHERE derecho.cuatrimestre = '$cuatrimestre'";
}
...
if ($resultado && mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_assoc($resultado)) {
        ...
        $nombreCompleto = $fila['derecho'] . ' ' . $fila['appe'] . ' ' . $fila['apme'];
        ...
        setText($pdf, "POR SER ALUMNO DE DERECHO", 190, 47, 138);
