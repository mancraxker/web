...
if (empty($cuatrimestre)) {
    ...
    $sql = "SELECT ingenieria.id, ingenieria.nombre AS ingenieria, ingenieria.app AS appe, ingenieria.apm AS apme 
            FROM ingenieria";
} else {
    // Si hay un cuatrimestre, se filtra por cuatrimestre
    $sql = "SELECT ingenieria.id, ingenieria.nombre AS ingenieria, ingenieria.app AS appe, ingenieria.apm AS apme 
            FROM ingenieria WHERE ingenieria.cuatrimestre = '$cuatrimestre'";
}
...
if ($resultado && mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_assoc($resultado)) {
        ...
        $nombreCompleto = $fila['ingenieria'] . ' ' . $fila['appe'] . ' ' . $fila['apme'];
        ...
        setText($pdf, "POR SER ALUMNO DE INGENIERIA", 190, 47, 138);
    ...
    echo "<script language='JavaScript'>
                alert('No se encontraron alumnos en el cuatrimestre seleccionado');
